...
export class AluraAccessService {
  ...
  getAll(filters: QueryParams | undefined) {
    ...
    if (filters) {
      if (filters.page) params = params.append('_page', filters.page.toString());
      if (filters.status) params = params.append('status', filters.status);
      if (filters.name) params = params.append('name_like', filters.name);
    }

    return this.http.get<Access[]>(this.apiUrl + '/alura-accesses', { params, observe: 'response' });
  }
